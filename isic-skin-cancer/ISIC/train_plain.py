...
data_dirs = [os.path.join(data["data_folder"], "processed/0_no_cancer"),
             os.path.join(data["data_folder"], "processed/1_cancer")
             ]
...
datasets, weights = dataloader.loadImageDataset(root_dirs=data_dirs, csv_file=os.path.join(data["data_folder"], "meta.csv"))
...
hist_dict['AUC'], hist_dict['F1 score'] = utils.get_auc_f1(
    model, datasets['test'])
